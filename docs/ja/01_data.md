...
> ### 本プロジェクトの特徴
>
> 本プロジェクトで実装するすべての要素（トークン化、Embedding、Self-Attention、Multi-Head、Layer Norm、残差接続、Cross-Entropy 学習、Alpaca 形式の instruction tuning）は、ChatGPT などで動いている実際の LLM の **基盤技術と共通** です。小さなモデルを使ってその技術に触れていきましょう。
...
> | **28** | 訓練サンプル数（コーパス 40 トークン − SEQ_LEN 12 = 28、§1.4 のスライドウィンドウで生成） |
> | **64** | 埋め込み次元（D_MODEL） |
> | **128** | FFN の隠れ層次元（D_FF） |
>
...
>
> 大規模なコーパスを扱う場合はデータを複数バッチに分割し、
> **バッチごとに forward → loss → backward → update** を繰り返します。
> ただし、各バッチ内で「次の単語を予測する」という学習の核は共通です。
