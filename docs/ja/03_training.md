# 第3章: 訓練 — モデルはどうやって賢くなるのか

![訓練](../images/chapter-03-training.png)

前章では入力がどう処理されて `logits`（各単語のスコア）になるかを見ました。
この章では、そのスコアと正解を比較し、パラメータを更新する
**訓練ループ** の仕組みを解説します。

---

## 3.1 訓練の全体像

第2章の最後で、モデルは 12 個の予測のうち何個かを外していました。
`tok_emb` や `Wq` といったパラメータが最初はランダムな値なので、当然の結果です。

訓練とは、この **外し方を少しずつ減らしていく作業** です。
やることは単純で、次の 4 つを繰り返すだけです。

1. **Forward** — いまのパラメータで、とにかく予測してみる
2. **Loss** — その予測が正解からどれだけ外れているかを、1 つの数値にまとめる
3. **Backward** — その数値を小さくするには、どのパラメータをどちらへ動かせばよいかを調べる
4. **Update** — 調べた方向へ、パラメータをほんの少しだけ動かす

![訓練ループの図。Forward で予測し、Loss でズレを数値にし、Backward で勾配を求め、Update でパラメータを動かす。この 4 ステップを 1 周（1 エポック）として 200 回くり返す。](../images/training-loop-ja.svg)

1 周するとパラメータがほんの少し「まし」になり、予測がわずかに正解へ近づきます。
1 回で劇的に賢くなるわけではなく、本プロジェクトではこれを
**200 回**（`EPOCHS = 200`）繰り返します。

②の「外れ具合をまとめた数値」を **損失（loss）**、
③の「どちらへ動かせばよいか」を **勾配（gradient）** と呼びます。
どちらもこの章で順に説明します。

---

## 3.2 コード全体

```python
def train(model, inputs, targets):
    optimizer = torch.optim.Adam(model.parameters(), lr=LR)

    for epoch in range(EPOCHS):
        logits = model.forward(inputs)               # ① Forward
        loss = F.cross_entropy(                       # ② Loss
            logits.view(-1, model.vocab_size),
            targets.view(-1),
        )

        optimizer.zero_grad()                         # 前回の勾配をリセット
        loss.backward()                               # ③ Backward（自動微分）
        optimizer.step()                              # ④ Update

        if (epoch + 1) % 20 == 0:
            print(f"epoch {epoch+1:4d}  loss={loss.item():.4f}")
```

>
> **Python Tips: `loss.item()` と f文字列**
>
> **`.item()`** はテンソルからPythonの普通の数値を取り出します：
> ```python
> t = torch.tensor(3.14)
> t          # → tensor(3.14)   ← テンソル型
> t.item()   # → 3.14           ← float型
> ```
>
> **f文字列** `f"..."` は変数を埋め込める文字列です。`:` の後は書式指定：
> ```python
> x = 42
> pi = 3.14159
> f"x={x:4d}"      # → "x=  42"      4桁の整数（右寄せ）
> f"pi={pi:.4f}"    # → "pi=3.1416"   小数点以下4桁
> ```

以下、各ステップを詳しく見ていきます。

---

## 3.3 ① Forward — 予測を出す

```python
logits = model.forward(inputs)   # (28, 12, 10)
```

前章で解説した処理そのものです。28サンプル（1バッチ）× 12 位置のそれぞれで、
10 単語の「スコア」が出力されます。

---

## 3.4 ② Cross-Entropy Loss — 予測と正解を比較する

### 何を計算しているか

```python
loss = F.cross_entropy(
    logits.view(-1, model.vocab_size),   # (336, 10) ← 28×12=336 予測
    targets.view(-1),                     # (336,)    ← 336 個の正解
)
```

この2行は少しわかりにくいので、丁寧に見ていきます。

### `logits` と `targets` の形状

Forward の結果 `logits` は3次元テンソルです：

```
logits の形状: (28, 12, 10)
                ↑   ↑   ↑
                |   |   └─ 10単語それぞれのスコア（次の単語の予測）
                |   └───── 12個の予測（入力の各単語について「次の単語」を予測）
               └───────── 28サンプル（この例の1バッチ）
```

第1章で見たように、入力と正解は「1つずらし」の関係でした：

```
入力:   the  cat  sat  on  the  mat   .  the  dog  sat  on  the
正解:   cat  sat  on   the mat   .   the dog  sat  on   the log
```

logits の各位置は、**その位置までの文脈を見て「次に来る単語」を予測したスコア** です。
例えば位置 3（"on"）の logits は、「"the cat sat on" の次に来る単語」の
10単語分のスコアです。正解は "the" です。

`targets` は、各位置に対応する正解の単語番号です：

```
targets の形状: (28, 12)
                 ↑   ↑
                 |   └─ 各位置の正解の単語番号（＝次に来るべき単語）
                 └───── 28サンプル（この例の1バッチ）
```

つまり、28サンプル（1バッチ）× 12位置 = **336 個の「次の単語」予測** があり、
それぞれに **1つの正解** があります。

### なぜ `view` で形状を変えるのか

`F.cross_entropy` は以下の形状を要求します：

- 第1引数：`(予測の数, クラス数)` — 2次元
- 第2引数：`(予測の数,)` — 1次元

しかし `logits` は `(28, 12, 10)` の3次元、`targets` は `(28, 12)` の2次元です。
そこで `view` を使って「28サンプル（1バッチ）× 12位置」を1列に並べます：

```
logits:  (28, 12, 10)  →  view(-1, 10)  →  (336, 10)
                                               ↑    ↑
                                              336個  10単語のスコア

targets: (28, 12)      →  view(-1)      →  (336,)
                                               ↑
                                              336個の正解番号
```

並ぶ順番は、サンプル0 の位置0〜11、次にサンプル1 の位置0〜11……と続きます。
**「どのサンプルのどの位置か」という区別をなくして、336 個の予測を一括で評価する**
ための並べ替えです。

>
> **Python Tips: `.view(-1, ...)` — `-1` は「自動計算」**
>
> `.view()` で `-1` を指定すると、他の次元から自動的にサイズを計算します：
> ```python
> x = torch.zeros(28, 12, 10)      # 28×12×10 = 3360 要素
>
> x.view(-1, 10)    # → shape: (336, 10)   -1 → 3360÷10 = 336
> x.view(-1)        # → shape: (3360,)     完全に1次元に平坦化
> ```

### Cross-Entropy Loss の意味

ある位置の logits が `[0.1, 2.5, 0.3, -0.1, ...]` で、
正解が単語番号 `1`（"the"）だったとします。

**Step 1: Softmax で確率に変換**

$$p_i = \frac{e^{\text{logit}_i}}{\sum_j e^{\text{logit}_j}}$$

```
logits:  [0.1,  2.5,  0.3, -0.1,  0.5, -0.2,  0.4, -0.1,  0.6,  0.0]
softmax: [0.05, 0.52, 0.06, 0.04, 0.07, 0.04, 0.06, 0.04, 0.08, 0.04]
                 ↑
              正解の "the" に 52% の確率 → まだ低い
```

**Step 2: 正解の確率の負の対数**

$$\text{loss} = -\log(p_{\text{correct}})$$

```
loss = -log(0.52) = 0.65
```

- 正解の確率が 1.0 に近い → loss ≈ 0（良い予測）
- 正解の確率が 0.0 に近い → loss → ∞（悪い予測）

### 損失の推移

実行結果を見ると（数値は実行ごとに変わりますが、傾向は同じです）：

```
epoch   20  loss=1.9469    ← ほぼランダムな予測（10単語の場合 -log(1/10) ≈ 2.30）
epoch   40  loss=1.5257
epoch   60  loss=0.8140
epoch   80  loss=0.5469
epoch  100  loss=0.3880    ← かなり正確に予測できている
epoch  120  loss=0.3099
epoch  140  loss=0.2568
epoch  160  loss=0.2227
epoch  180  loss=0.1862
epoch  200  loss=0.1147    ← ほぼ完璧に予測できている
```

loss が 2.3 付近（ランダム）から 0.11（ほぼ正解）へと下がっています。

---

![花壇の苗に水をあげるAliceとBob](../images/chapter-03-break.png)

## 3.5 ③ Backward — 誤差逆伝播（Backpropagation）

```python
optimizer.zero_grad()
loss.backward()
```

### 「勾配」とは何か

> 勾配の数学的な詳細は [補足: 勾配の数学的な直感](03a_gradient.md) で
> 具体的な数値例とともに解説しています。

損失 `loss` は全パラメータの関数です。
「あるパラメータを少し増やしたら loss はどれだけ変わるか」——
これが **勾配（gradient）** です。

数式で書くと、パラメータ $\theta$ に対する勾配：

$$\frac{\partial \text{loss}}{\partial \theta}$$

- 勾配が **正** → そのパラメータを **減らせば** loss が下がる
- 勾配が **負** → そのパラメータを **増やせば** loss が下がる

### 逆伝播の仕組み

`loss.backward()` は **連鎖律（chain rule）** を使って、
出力から入力に向かって勾配を逆方向に伝播させます。

```
tok_emb → Embedding → Attention → FFN → logits → loss
  ←─────────────────────────────────────────────────
          backward: 勾配が loss から逆向きに流れる
```

連鎖律とは：

$$\frac{\partial \text{loss}}{\partial W_q} = \frac{\partial \text{loss}}{\partial \text{logits}} \cdot \frac{\partial \text{logits}}{\partial \text{attn}} \cdot \frac{\partial \text{attn}}{\partial W_q}$$

各層の局所的な微分を掛け合わせるだけで、全パラメータの勾配が求まります。

> **PyTorch の autograd**: `loss.backward()` 一行で、forward で行った
> 全計算の勾配を自動的に求めてくれます。本プログラムでは forward を
> 手書きし、backward は PyTorch に任せています。

### `zero_grad()` の必要性

PyTorch は勾配を **累積** するため、毎回リセットしないと前回の勾配が
残ってしまいます。

---

## 3.6 ④ Update — パラメータを更新する

```python
optimizer.step()
```

### 基本の更新則（勾配降下法）

$$\theta \leftarrow \theta - \eta \cdot \frac{\partial \text{loss}}{\partial \theta}$$

$\eta$（学習率、`LR = 0.001`）は「1回の更新でどれだけ動かすか」です。

```
例：
  W_q の要素が 0.05 で、勾配が +0.2 のとき
  → 新しい値 = 0.05 - 0.001 × 0.2 = 0.0498
  → loss を下げる方向に少しだけ動いた
```

### Adam Optimizer

本プログラムでは単純な勾配降下法ではなく **Adam** を使っています。
Adam は各パラメータごとに学習率を自動調整する手法で、収束が速いです。

- 頻繁に更新されるパラメータ → 学習率を小さく
- あまり更新されないパラメータ → 学習率を大きく

---

## 3.7 学習の全過程を追う

![学習の全過程。Epoch 1 ではパラメータがランダムで loss 約 2.3、Epoch 100 で約 0.39、Epoch 200 で約 0.11 まで下がり、予測がほぼ正解になる。ループの中身は最初から最後まで同じ。](../images/training-progress-ja.svg)

パラメータの値が変わっていくだけで、ループの中身（Forward → Loss → Backward → Update）は
最初から最後まで一度も変わりません。

---

## 3.8 学習されるもの

本プログラムのパラメータ一覧：

| パラメータ | 形状 | 何を学ぶか |
|-----------|------|-----------|
| `tok_emb` | (10, 64) | 各単語のトークン埋め込み |
| `pos_emb` | (12, 64) | 各位置の位置埋め込み |
| `Wq, Wk, Wv` | (64, 64) ×3 ×2層 | Attention の問い合わせ方 |
| `Wo` | (64, 64) ×2層 | Attention 出力の統合方法 |
| `W1, b1` | (64,128), (128,) ×2層 | FFN の変換（前半） |
| `W2, b2` | (128,64), (64,) ×2層 | FFN の変換（後半） |
| `ln*_g, ln*_b` | (64,) 各種 | Layer Norm のスケールとシフト |

これらがすべて、200 回の訓練で **loss を最小にする方向** に
少しずつ調整されていきます。

---

## まとめ

```
Forward（順伝播）:  inputs → 予測(logits)
                          ↓
Loss（損失計算）:    予測と正解の差 → 1つの数値
                          ↓
Backward（逆伝播）:  全パラメータの勾配を自動計算
                          ↓
Update（更新）:      勾配を使ってパラメータを微調整
                          ↓
                    繰り返し → 損失がどんどん小さくなる
```

| 概念 | 意味 |
|------|------|
| 損失（loss） | 予測がどれだけ間違っているかの指標 |
| 勾配（gradient） | パラメータを動かす方向と大きさ |
| 逆伝播（backprop） | 連鎖律で勾配を効率的に計算する手法 |
| 学習率（LR） | 1回の更新でどれだけ動かすか |
| エポック（epoch） | 訓練データを1周すること |
