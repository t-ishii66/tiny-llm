...
>
> Every element we implement in this project (tokenization, Embedding, Self-Attention, Multi-Head, Layer Norm, residual connections, Cross-Entropy training, Alpaca-style instruction tuning) shares the **same foundational technology** as the real LLMs running behind ChatGPT and the like. Let's get hands-on with that technology by using a small model.
...
> | **10** | Vocabulary size (vocab_size) — extended to **15** in Chapter 5 with the Alpaca markers added |
> | **12** | Sequence length (SEQ_LEN) |
> | **16** | Head dimension (head_dim = 64 ÷ 4) |
> | **28** | Number of training samples (corpus 40 tokens − SEQ_LEN 12 = 28, generated by the sliding window in §1.4) |
...
> **Difference from real LLMs:** GPT and similar models further split words into
...
>
> This `(28, 12)` is the form that bundles "28 samples into one chunk (1 batch)"
> for processing together. In other words, this project keeps
> **the batch count at 1** to simplify the explanation.
>
> When dealing with a large corpus, the data is split into multiple batches,
> and **forward → loss → backward → update** is repeated **for each batch**.
> Still, the core learning idea — "predict the next word" — is the same inside every batch.
