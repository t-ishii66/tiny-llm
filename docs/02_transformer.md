...
self.pos_emb = param(SEQ_LEN, D_MODEL)      # (16, 64)
```
...
```python
    Q = x @ Wq   # (24, 16, 64) @ (64, 64) → (24, 16, 64)
    K = x @ Wk   # 同上
    V = x @ Wv   # 同上
```
...
```python
    ...
    # (24, 16, 64) → (24, 16, 4, 16) → (24, 4, 16, 16)
```
...
```python
    ...
    # (24, 4, 16, 16) @ (24, 4, 16, 16)^T → (24, 4, 16, 16)
```
...
```python
    out = attn_weights @ V   # (24, 4, 16, 16) @ (24, 4, 16, 16) → (24, 4, 16, 16)
```
...
```python
    out = out.transpose(1, 2).contiguous().view(B, T, D)  # → (24, 16, 64)
    out = out @ Wo                                          # 出力射影
```
...
logits = x @ self.tok_emb.T   # (24, 16, 64) @ (64, 10) → (24, 16, 10)
```
...
```
token_ids (24, 16)            ← 整数（単語番号）
    ↓ Embedding
x (24, 16, 64)               ← 各単語が64次元ベクトルに
    ↓ Self-Attention
x (24, 16, 64)               ← 文脈を考慮したベクトルに変化
    ↓ FFN
x (24, 16, 64)               ← さらに変換
    ↓ ×2 blocks
x (24, 16, 64)               ← 2層分の処理
    ↓ Layer Norm + tok_emb.T
logits (24, 16, 10)           ← 各位置で10単語分のスコア
```

| コンポーネント | 形状変化 | 役割 |
|---------------|----------|------|
| Token Embedding | (24,16) → (24,16,64) | 単語を数値ベクトルに |
| Positional Emb. | 加算 | 位置情報を付加 |
| Self-Attention | (24,16,64) → (24,16,64) | 単語間の関係を捉える |
| Layer Norm | 形状不変 | 値を安定させる |
| Feed-Forward | (24,16,64) → (24,16,64) | 各単語の表現を変換 |
| 出力射影 | (24,16,64) → (24,16,10) | ベクトルを語彙スコアに |
